...
def define_values_opt(i):
    ...
    return ext_in, ext_out, k_increment
...
def read_write_monitor(folder_speed, folder_monitor, mpi_mnt_id, folder_plot, ext_in, ext_out, k_increment):
    import numpy as np
    import os
    ...
    for i in range(len(mpi_mnt_id)):
        ...
        if os.path.isfile(filename2):
            ...
            for j in range(num_of_mon):
                time = val_monitor[:, 0]
                values = val_monitor[:, 1 + k:k_increment + k + 1]
                datafilename = folder_speed + '/' + folder_monitor + "/site" + '{0:05d}'.format(
                    id_of_mon[j]) + "." + ext_out
                write_monitor(time, values, datafilename)
                k = k + k_increment
    return num_of_tot_mon
...
def rewrite_monitor_format(folder_speed, folder_plot):
    ...
    for iopt in range(len(out_opt)):
        ext_in, ext_out, k_increment = define_values_opt(iopt)
        if out_opt[iopt] == 1:
            nmonitors[iopt] = read_write_monitor(folder_speed, folder_monitor, mpi_mnt_id, folder_plot, ext_in,
                                                 ext_out, k_increment)
    return nmonitors.astype(int), folder_monitor
...
def define_selected_time_history(selected_code, folder_simulation, nobs, j, comp, sites, folder_plot, fmin, fmax):
    ...
    if selected_code == 'speed':
        nmonitors, folder_monitor = rewrite_monitor_format(folder_simulation, folder_plot)
        for jj in range(len(nmonitors)):
            for iobs in range(nmonitors[jj]):
                if iobs == nobs:
                    ext_in, ext_out, k_increment = define_values_opt(jj)
                    if ext_in == 'D':
                        ext_out = 'd'
                    elif ext_in == 'V':
                        ext_out = 'v'
                        ...
                        sys.exit('SPEED: Plot not supported for defined input data')
                    datafilename = folder_simulation + '/' + folder_monitor + "/site" + '{0:05d}'.format(
                        iobs + 1) + "." + ext_out
                    time_series = np.loadtxt(datafilename)
                    time = time_series[:, 0]
                    dt = time[1] - time[0]
                    if comp == 'NS':
                        column = 2
